...
const Select: React.FC<SelectProps> = ({ label = "", name, value = "", options, className = "", onChange }) => {
    ...
    const selectTriggerClassName = "text-xs md:text-base transition-all duration-200 w-full px-3 py-2 dark:text-white bg-white/50 dark:bg-black/50 rounded-md shadow-sm backdrop-blur-sm focus:outline-none focus:ring-2 focus:ring-primary-500 focus:border-primary-500 flex justify-between items-center cursor-pointer space-x-2";
    const listClassName = "flex flex-col absolute z-50 w-full bg-white/80 dark:bg-secondary-900/80 backdrop-blur-2xl rounded-md shadow-lg overflow-y-auto scrollbar transition-all duration-300";
